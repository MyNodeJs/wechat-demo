...
var handler = weChat(config.wechat, weChat.text(function (info, req, res, next) {
    ...
    if (/约/.test(content)) {
        ...
        console.log("name\n", name);
        request('http://114.215.159.50:1400/artist/find?name=' + name, function (error, response, body) {
            var uid = body.uid;
            request('http://114.215.159.50:1400/artist/getArtistPics?uid=' + uid, function (error, resposne, pics) {
                var arr = [];
                pics = JSON.parse(pics);
                pics.forEach(function (item, index) {
                    var obj = {
                        title: body.name,
                        description: body.popMusic[index] || body.name,
                        picurl: item.src,
                        url: item.src
                    };
                    arr.push(obj);
